...
export function SocialIntegrationModal({ onClose }) {
  ...
  const mergeMut = useMutation({
    mutationFn: () => candidateService.syncMergeSocialProfile(githubData),
    onSuccess: () => {
      alert('GitHub profile data merged into candidate profile database successfully!');
      queryClient.invalidateQueries({ queryKey: ['candidateProfile'] });
      onClose();
    },
    onError: (err) => {
      alert(err.response?.data?.message || 'Failed to merge social profile.');
    },
  });

  const handleFetch = () => {
    if (!hasPermission) {
      alert('You must check explicit permission consent before fetching your social profile.');
      return;
    }
  ...
  return (
    <div className="fixed inset-0 z-50 bg-slate-900/40 backdrop-blur-xs flex items-center justify-center p-4 overflow-y-auto">
      <div className="bg-white rounded-3xl border border-slate-200 shadow-2xl p-6 md:p-8 max-w-2xl w-full space-y-6 text-xs text-slate-900 my-8">
        {/* Header */}
        <div className="flex items-start justify-between border-b border-slate-100 pb-4">
          <div>
            ...
            <p className="text-xs text-slate-500">
              Fetch public repositories, programming languages breakdown, pinned repos, total star counts, and README summaries.
            </p>
          </div>
        ...
        <div className="p-4 rounded-2xl bg-indigo-50/70 border border-indigo-200 space-y-2">
          <label className="flex items-start gap-3 cursor-pointer">
            ...
            <span className="font-bold text-indigo-950 text-xs leading-snug">
              I explicitly grant permission to AI ATS to fetch, analyze, and merge my public GitHub repositories data into my profile.
            </span>
          </label>
        </div>
        ...
        <div className="space-y-4">
          <div>
            ...
            <div className="flex gap-2">
              ...
              <button
                onClick={handleFetch}
                disabled={!hasPermission || fetchGithubMut.isPending}
        ...
        <div className="pt-3 border-t border-slate-100">
          ...
          <div className="p-5 rounded-2xl bg-slate-50 border border-slate-200 space-y-4">
            ...
            {!githubData ? (
              <p className="text-slate-400 py-6 text-center">Enter your username and click "Fetch Profile Data" to load GitHub details.</p>
            ) : (
        ...
        <div className="pt-3 border-t border-slate-100 flex items-center justify-end gap-3">
          ...
          <button
            ...
            disabled={!githubData || mergeMut.isPending}
